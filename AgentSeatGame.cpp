...
AgentSetGame::AgentSetGame(const InitData& init) : IScene(init) {
	cards = new CardDeck();
...
AgentSetGame::~AgentSetGame()
{
}

void AgentSetGame::update() {
...
void AgentSetGame::cardsDraw() const {//カードの描画
	for (int i = 0; i < 60; i++) {
...
void AgentSetGame::hintDraw() const {
	for (int i = 0; i < 60; i++) {
		const Vec2 center(15 + i % 15 * (cards->pack.width() + 15), 15 + (i / 15) * (cards->pack.height() + 15));
		plotCards->pack(plotCards->getCard(i)).draw(center);
	}
...
void AgentSetGame::plotDraw() const {
...
void AgentSetGame::buttonDraw() const {
...
void AgentSetGame::numberDraw() const {//数字の表示
